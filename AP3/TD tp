...
void insert(tree_t *a, s_base_t val)
{
  ...
  {
    if (val.prix < value(*a).prix)
    {
      tree_t arbre_temp = left(*a);
      insert(&arbre_temp, val);
      change_left(*a, arbre_temp);
    }
    else
    {
      tree_t arbre_temp = right(*a);
      insert(&arbre_temp, val);
      change_right(*a, arbre_temp);
    }
  }
}
...
// On l'utilise pour remove_value
void remove_max(tree_t *a)
{
  if (is_empty(right(*a)))
  {
    tree_t arbre_temp = *a;
    *a = left(*a);
    change_left(arbre_temp, cons_empty());
    free_tree(arbre_temp);
  }
  else
  {
    tree_t arbre_temp = right(*a);
    remove_max(&arbre_temp);
    change_right(*a, arbre_temp);
  }
}
...
{
  // On suppose que la valeur est présente, et donc l'arbre n'est pas vide.
  if (!is_empty(*a))
  {
    if (compare(value(*a), val) > 0)
    {
      tree_t arbre_temp = left(*a);
      remove_value(&arbre_temp, val);
      change_left(*a, arbre_temp);
    }
    ...
    // La racine est la valeur à supprimer.
    else
    {
      ...
      {
        change_value(*a, find_max(left(*a)));
        tree_t arbre_temp = left(*a);
        remove_max(&arbre_temp);
      }
    }
...
// q5
void rotation_droite(tree_t *a)
{
  tree_t arbre_temp = left(*a);
  change_left(*a, right(arbre_temp));
  change_right(arbre_temp, *a);
  *a = arbre_temp;
}

void rotation_gauche(tree_t *a)
{
  tree_t arbre_temp = right(*a);
  change_right(*a, left(arbre_temp));
  change_left(arbre_temp, *a);
  *a = arbre_temp;
}
...
void insert_AVL(tree_t *a, s_base_t val)
{
  ...
  int difference_hauteur = height(left(*a)) - height(right(*a));
  if (difference_hauteur > 1 && val.prix < value(*a).prix)
  {
    rotation_droite(a);
  }
  if (difference_hauteur > 1 && val.prix >= value(*a).prix)
